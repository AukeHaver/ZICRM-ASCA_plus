...
design_info <- gen_sim_info(n_treatments = 5,
                            ...
                            family = "zipois",
                            prob_zero = 0.5,
                            jackknife_fold = 7) # Which type of family do we want to simulate
...
Tab<-c(0,.02,-.01,-.02, 0.01,         3,1, -.5,-1.5,-2,      4,-.5,-1,-1.5,-1,
       3.05,-.5,-1.5,-.5,-1.5,  .3,0,.4,.2,.2,      -0.05,.3, -0.3 ,.1,-0.2)%>%
...
# Time EFFECT 
  ...
  annotate_figure(top=text_grob("Time Effect (A)",
                                size=30,
                                family="Times")) %>%
  ggsave(filename=paste0("../Images/",Sys.Date(),"_ZIGM_ASCA_Time_before.png"),
         plot=.,
         width = 8,
         height= 8)
...
ggarrange(
  ...
  annotate_figure(top=text_grob("Interaction Effect (AB)",
                                size=30,
                                family = "Times")) %>%
  ggsave(filename=paste0("../Images/",Sys.Date(),"_ZIGM_ASCA_Interaction_before.png"),
         plot=.,
         width = 8,
         height= 8)
```
...
gen_long_response_df(unbalanced_simulation_matrices) %>%
  ...
  annotate_figure(bottom = text_grob("zOTU Count",family = "Times",size=20),
                  left = text_grob("Frequency (counts)",
                                   ...
                                   family="Times",
                                   rot=90)) %>%
  ggsave(filename=paste0("../Images/",Sys.Date(),"_ZIGM_ASCA_Zero-inflation_per_zOTU.png"),
         plot=.,
         width = 10,
         height= 4)
...
# Subject effect
full_dataset_pca_results <- 
  glmm_fit_results %>%
  comp_mat_from_coef2(design_info$X,
                      include_residuals = TRUE) %>% 
  pca_effect_matrices()

expl_var_Mres <-
  full_dataset_pca_results$Mres$singular_values %>% 
  expl_var_from_svd()

design_info$design %>%
  cbind(full_dataset_pca_results$Mres$scores) %>%
  ggplot(aes(x=`1`,
             y=`2`,
             color=treatment,
             shape=timepoint))+
  geom_point()
full_dataset_pca_results$Mres$scores[,1] %*% full_dataset_pca_results$Mres$scores[,2]
design_info$design %>%
  cbind(full_dataset_pca_results$Mres$scores) 

full_dataset_pca_results$Mres$loadings[,1] %>% plot()

mean(comp_mat_from_coef2(glmm_fit_results,design_info$X)$Ma_ab[,1])
mean(simulateResiduals(glmm_fit_results$model[[1]])$scaledResiduals)
```
...
ggarrange(
  ...
  mutate(var = factor(var,levels=seq(1:16)))%>%
    arrange(var)%>%
  ggplot(aes(x=var,
             y=median))+
  geom_bar(stat="identity",
