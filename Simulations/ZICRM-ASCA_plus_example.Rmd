...
ggarrange(
  ...
  annotate_figure(top=text_grob("Time Effect (A)",
                                ...
                                family="Times")) %>%
  # Export to pdf
  ggexport(filename=paste0("../Images/",
                         Sys.Date(),
                         "_ZIGM_ASCA_Time_before.pdf"),
         plot=.,
         width = 8,
         height= 8)
...
ggarrange(
  ...
  ggexport(filename=paste0("../Images/",
                         Sys.Date(),
                         "_ZIGM_ASCA_Interaction_before.pdf"),
         plot=.,
         width = 8,
         height= 8)
```
...
gen_long_response_df(unbalanced_simulation_matrices) %>%
  ...
  ggexport(filename=paste0("../Images/",
                         Sys.Date(),
                         "_ZIGM_ASCA_Zero-inflation_per_zOTU.pdf"),
         plot=.,
         width = 6,
         height= 6)
...
ggarrange(
  ...
  glmm_jackknife_results$Tab %>% 
      ...
      filter(PC==2) %>%
      group_by(timepoint,treatment)%>%
      mutate(quantile_025 = -quantile(score,.975),
             quantile_500 = -quantile(score,.500),
             quantile_975 = -quantile(score,.025)) %>%
    ggplot(aes(x=timepoint,
               y=quantile_500,
               color=treatment))+
    geom_point(size=5)+
  ...
  glmm_jackknife_results$Pab %>% filter(PC==2)%>%
      group_by(var)%>%
      mutate(quantile_025 = -quantile(loading,.975),
             quantile_500 = -quantile(loading,.500),
             quantile_975 = -quantile(loading,.025)) %>%
      select(-loading)%>%
    distinct()%>%
  ggplot(aes(x=var,
             y=quantile_500))+
  ...
  annotate_figure(top=text_grob("Interaction Effect (AB)",
                                family="Times",
                                size=30)) %>%
  ggexport(filename=paste0("../Images/",Sys.Date(),"_AB_",design_info$prob_zero,"_REP_",design_info$n_subjects,".pdf"),
         plot=.,
         width = 8,
         height= 8)
```
